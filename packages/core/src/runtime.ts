...
import { ChannelType, MemoryType, ModelType, ServiceType } from './types';
...
export class AgentRuntime implements IAgentRuntime {
  ...
  constructor(opts: {
    ...
    // Add the created InstrumentationService instance to the services map
    if (this.instrumentationService && this.instrumentationService.isEnabled()) {
      // Check if enabled before adding
      this.services.set(
        ServiceType.INSTRUMENTATION,
        this.instrumentationService as unknown as Service
      );
      this.runtimeLogger.debug(
        `Instrumentation service registered with key: ${ServiceType.INSTRUMENTATION}`
      );
    } else {
      this.runtimeLogger.debug(
        `Instrumentation service not registered to map (disabled or failed)`
      );
    }
  ...
  async useModel<T extends ModelTypeName, R = ModelResultMap[T]>(
    ...
    // if (promptContent) { span.addEvent('llm.prompt', { 'prompt.content': promptContent }); }
    ...
    try {
      ...
      this.runtimeLogger.debug(
        ...
        Array.isArray(response)
          ...
          : JSON.stringify(response, safeReplacer()) // Added safeReplacer here
      );
