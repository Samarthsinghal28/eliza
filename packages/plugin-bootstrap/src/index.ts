...
const messageReceivedHandler = async ({
  ...
  logger.debug('[Bootstrap] ENTERED messageReceivedHandler');
  ...
  logger.debug(
    `[Instrumentation] Service obtained: ${!!instrumentationService}, Tracer obtained: ${!!tracer}, Enabled: ${instrumentationService?.isEnabled()}`
  );

  if (!tracer || !instrumentationService?.isEnabled()) {
    // Instrumentation disabled, run original logic directly
    logger.debug('[Instrumentation] Disabled or tracer not found. Skipping span creation.');
    await _messageReceivedHandlerLogic({ runtime, message, callback, onComplete });
    return;
  }

  logger.debug('[Instrumentation] Tracer found and enabled. Starting root span...');
  // Wrap the main logic in a root span
  await tracer.startActiveSpan('AgentRuntime.handleMessage', async (rootSpan) => {
    logger.debug('[Instrumentation] Entered startActiveSpan block.');
    try {
      ...
      logger.debug('[Instrumentation] Setting attributes:', attributes);
      ...
      logger.debug('[Instrumentation] Calling _messageReceivedHandlerLogic...');
      await _messageReceivedHandlerLogic({ runtime, message, callback, onComplete }, rootSpan); // Pass span down
      logger.debug('[Instrumentation] _messageReceivedHandlerLogic finished successfully.');

      // Set success status
      rootSpan.setStatus({ code: SpanStatusCode.OK });
      rootSpan.addEvent('processing_succeeded');
      logger.debug('[Instrumentation] Set span status to OK.');
    } catch (error) {
      // Record exception and set error status
      logger.error('[Instrumentation] Error caught in span block:', error);
      ...
      logger.debug('[Instrumentation] Ending root span.');
      rootSpan.end();
    }
  });
...
const _messageReceivedHandlerLogic = async (
  { runtime, message, callback, onComplete }: MessageReceivedHandlerParams,
  rootSpan?: any /* opentelemetry Span */
): Promise<void> => {
  ...
  if (!message.metadata) {
    message.metadata = { type: MemoryType.MESSAGE };
  }
